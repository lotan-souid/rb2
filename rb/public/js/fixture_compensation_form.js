const FIXTURE_BASEMAP_FALLBACK = {
  default: {
...
let __fixture_basemaps = null;
let __fixture_map_cfg = null;
let __fixture_bg_key = 'default';
let __fixture_map;
let __fixture_geo_layer;
let __fixture_leaflet_loading;
let __fixture_base_layer;

frappe.ui.form.on('Fixture Compensation', {
  refresh(frm) {
    ...
    frm.add_custom_button(__('Cahnge To Background A'), () => toggle_fixture_background(frm, 'A'), __('GIS Actions'));
    frm.add_custom_button(__('Cahnge To Background B'), () => set_fixture_background(frm, 'B'), __('GIS Actions'));
    ...
    ensure_fixture_client_cfg(() => setTimeout(() => set_geolocation_basemap(frm, 'location', __fixture_bg_key), 300));
  }
});
...
function fetch_geometry_from_gis(frm) {
  ...
  frappe.call({
    method: 'rb.gis_integration.api.fetch_fixture_compensation_geometry',
    args: { fixture_name: frm.doc.name },
    callback: (r) => {
      try {
        if (r && r.message) {
...
function sync_all_geometries(frm) {
  frappe.confirm(__('This will sync geometries for all Fixture Compensation records. Continue?'), () => {
    frappe.call({
      method: 'rb.gis_integration.api.sync_all_fixture_compensation_geometries',
      args: {},
      freeze: true,
...
function ensure_fixture_client_cfg(cb) {
  if (__fixture_basemaps) {
    cb && cb();
    return;
  }
  frappe.call({ method: 'rb.gis_integration.api.gis_reload_config' })
    .always(() => {
      frappe.call({
        ...
        args: { doctype: 'Fixture Compensation' },
        callback: (r) => {
          const msg = r.message || {};
          __fixture_basemaps = msg.basemaps || null;
          __fixture_map_cfg = msg.map || {};
          __fixture_bg_key = normalize_bg_key((__fixture_map_cfg && __fixture_map_cfg.default_basemap) || 'default');
          cb && cb();
        },
        error: () => {
          __fixture_basemaps = null;
          __fixture_map_cfg = null;
          __fixture_bg_key = 'default';
          cb && cb();
        }
      });
    });
...
function get_fixture_basemap(which) {
  const key = normalize_bg_key(which);
  if (__fixture_basemaps && __fixture_basemaps[key]) return __fixture_basemaps[key];
  if (__fixture_basemaps && __fixture_basemaps.default) return __fixture_basemaps.default;
  return FIXTURE_BASEMAP_FALLBACK[key] || FIXTURE_BASEMAP_FALLBACK.default;
}

function ensure_map(frm) {
  ensure_leaflet(() => {});
  const id = 'fixture-geo-map';
  ...
  if (!__fixture_map) {
    ...
    if (el && window.L) {
      __fixture_map = L.map(id);
      ensure_fixture_client_cfg(() => { set_map_basemap(__fixture_map, __fixture_bg_key); apply_default_view(__fixture_map); });
    }
  }
  return __fixture_map;
}

function render_geometry_on_form(frm, featureCollection) {
  const map = ensure_map(frm);
  if (!map || !window.L) return;
  if (__fixture_geo_layer) {
    __fixture_geo_layer.remove();
    __fixture_geo_layer = null;
  }
  __fixture_geo_layer = L.geoJSON(featureCollection, { style: { color: '#117864', weight: 2, fillOpacity: 0.2 } }).addTo(map);
  try {
    if (should_fit_geometry()) {
      map.fitBounds(__fixture_geo_layer.getBounds(), { padding: [10, 10] });
    } else {
...
function ensure_leaflet(cb) {
  ...
  if (__fixture_leaflet_loading) return;
  __fixture_leaflet_loading = true;
  ...
  script.onload = () => { __fixture_leaflet_loading = false; cb && cb(); };
  script.onerror = () => { __fixture_leaflet_loading = false; console.warn('Leaflet load failed'); };
...
function set_geolocation_basemap(frm, fieldname, which) {
  ...
  if (!field) return;
  const bm = get_fixture_basemap(which);
  const swap = () => {
...
function set_map_basemap(map, which) {
  const bm = get_fixture_basemap(which);
  try {
    if (__fixture_base_layer && map.hasLayer && map.hasLayer(__fixture_base_layer)) {
      map.removeLayer(__fixture_base_layer);
    }
  } catch (e) {}
  __fixture_base_layer = L.tileLayer(bm.url, bm.options || { maxZoom: 19 }).addTo(map);
}

function toggle_fixture_background(frm, key) {
  const target = normalize_bg_key(key || 'A');
  __fixture_bg_key = (__fixture_bg_key === target) ? 'default' : target;
  const map = ensure_map(frm);
  if (map && window.L) set_map_basemap(map, __fixture_bg_key);
  set_geolocation_basemap(frm, 'location', __fixture_bg_key);
}

function set_fixture_background(frm, key) {
  const norm = normalize_bg_key(key || 'B');
  __fixture_bg_key = norm;
  const map = ensure_map(frm);
  if (map && window.L) set_map_basemap(map, __fixture_bg_key);
  set_geolocation_basemap(frm, 'location', __fixture_bg_key);
}
...
function should_fit_geometry() {
  try {
    if (__fixture_map_cfg && typeof __fixture_map_cfg.fit_to_geometry === 'boolean') {
      return __fixture_map_cfg.fit_to_geometry;
    }
  } catch (e) {}
  return true;
}
...
function open_tiles_map(frm) {
  const layer = (__fixture_map_cfg && __fixture_map_cfg.tileserv_layer) || 'rb_layers.fixture_compensation';
